...
const ExpiryProductListScreen = ({ route, navigation }) => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(`http://192.168.24.17:3000/expiry-products/${userId}`);
    ...
    <View style={styles.container}>
      ...
      {filteredProducts.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.suggestionText}>Você não possui nenhum alimento vencido!</Text>
        </View>
      ) : (
        <FlatList
